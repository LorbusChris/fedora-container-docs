...
For most layered images built by the https://fedoraproject.org/wiki/Changes/Layered_Docker_Image_Build_Service[Fedora Layered Docker Image Build Service], the FROM line will use one of the Fedora base images that exist on the https://registry.fedoraproject.org/[Fedora Container Registry]:
